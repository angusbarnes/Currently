...
def test_modbus_logs():
    ...
    with open('validity_results.csv', 'w', newline='') as outfile:
        ...
        writer.writerow([
            "timestamp", "node_name",
            "ref_v", "verify_v", "simulated_v",
            "ref_i", "verify_i", "simulated_i",
        ])
        for reading_set in database.fetch_batches("../sensitive/modbus_data.db", "2023-12-29 04:45:00"):
            ...
            reference_line, reference_bus = evaluate_load_flow_with_known_loads(nodes, lines, net, reading_set, site_totals, total_rating)
            clear_network_loads(net)
            verify_line, verify_bus = evaluate_load_flow_with_known_loads(nodes, lines, net, reading_set, site_totals, total_rating)
            clear_network_loads(net)
            network_sim_line, network_sim_bus = evaluate_load_flow_with_known_loads(nodes, lines, net, reading_set, site_totals, total_rating, simulate_network=True, batch_allocate=False)
            ...
            for bus_idx, ref_row in reference_bus.iterrows():
                ...
                if 'from_bus' in reference_line.columns:
                    ...
                    sim_lines = network_sim_line[network_sim_line['from_bus'] == bus_idx]
                ...
                writer.writerow([
                    timestamp, node_name,
                    ref_v, verify_v, sim_v,
                    ref_i, verify_i, sim_i,
                ])

def evaluate_load_flow_with_known_loads(nodes, lines, net, reading_set, site_totals, total_rating, simulate_network=False, batch_allocate=False):
