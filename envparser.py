...
def _load_from_file(path: pathlib.Path, 
                    seed_dict: typing.Dict[str, str] = None, 
                    override = False) -> typing.Dict[str, str]:
    """
    ...
    with open(path, 'r', encoding='utf-8') as file:
        for i, line in enumerate(file):
            ...
            if not text or text[0] == '#': continue

            if "=" not in text or text.count("=") != 1:
                raise SyntaxError(f"Syntax error in environment file. There is an incorrect number of '=' characters. Line #{i+1} in '{path.absolute()}'")
            
            key, value = text.split('=')

            if key in key_value_pairs and not override:
                raise KeyError(f"Re-definition of environment variable '{key}' in file '{path.absolute()}' on line #{i+1}.")
            else:
    ...
    if resolved_path.is_file():
        ...
        for path in resolved_path.glob("*.env"):
            if path.is_file() and path.name.endswith(".env"):
                ...
                context_kv_dict = _load_from_file(path, context_kv_dict, override=override)
